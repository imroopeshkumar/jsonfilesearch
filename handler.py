...
menuoptions1=[ 'Press 1 to begin term search', 'Press 2 to clear search file','Type \'terms\' to lookup searchable terms', 'Type help to view help', 'Type quit to exit']
    ...
    for file in fileslist:
        if(not file.name==searchfile):
            for result in results:
                result.subresults +=search(file,list(result.item.values()),list(file.content[0].keys()),  result.item  )
...
def printdetails(obj):
    for x, y in obj.items():
            print('{0:10}\t\t {1}\n'.format(str(x).strip(),str(y).strip()))
...
def runfullsearch(state):
    results = []
    vals = processinput('enter values to search. if more than one value, separate by \',\'').split(',')

    for file in state.searchfiles:
        results+=search(file,file.searchableterms, vals )
